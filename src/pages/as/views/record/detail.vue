        ...
        <Scroll height="500">
          <div class="steps-content" v-for="item in nodeList" :key="item.index">
            ...
            <div class="img-content">
              ...
              <vue-photo-zoom-pro v-if="item.vcInfraPicturePath" :url="item.vcInfraPicturePath"  :width="180" type="circle"></vue-photo-zoom-pro>
              <!-- <img v-if="item.vcNormalPicturePath" :src="item.vcNormalPicturePath" alt /> -->
              <vue-photo-zoom-pro v-if="item.vcNormalPicturePath" :url="item.vcNormalPicturePath" :width="180"></vue-photo-zoom-pro>
            </div>
            <i class="line"></i>
            <i class="dot"></i>
    ...
    getStepsDetail() {
      ...
      ctx.axios.getStepDetail({
        ...
        for (var i = 0, len = data.length; i < len; i++) {
          ...
          data[i].iUpdateTime = data[i].iUpdateTime ? moment(data[i].iUpdateTime).format('HH:mm:ss') : '--'
        }
        ctx.nodeList = data
      }).catch(err => {
        console.log(err)
